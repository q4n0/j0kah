...
func checkSystemCompatibility() {
	fmt.Println("\033[1;33mPerforming system compatibility check...\033[0m")

	// Check for OS and architecture compatibility
	fmt.Printf("\033[1;33mOperating System: %s\033[0m\n", runtime.GOOS)
	fmt.Printf("\033[1;33mArchitecture: %s\033[0m\n", runtime.GOARCH)

	if runtime.GOOS != "linux" && runtime.GOARCH != "amd64" {
		fmt.Printf("\033[1;31mError: Unsupported OS or Architecture. You’re not running this monstrosity on a suitable platform. Try again on a supported system: Linux amd64.\033[0m\n")
		os.Exit(1)
	} else {
		fmt.Println("\033[1;32mSystem compatibility check passed. You’re ready for the madness!\033[0m")
	}
}
...
func main() {
	...
	if saveResults {
		...
		err := os.WriteFile(fileName, []byte(results), 0644)
		if err != nil {
			fmt.Printf("\033[1;31mFailed to save scan results to file: %s. Did you break the file system?\033[0m\n", err)
		} else {
			fmt.Printf("\033[1;32mScan results have been saved to %s.\033[0m\n", fileName)
		}
	}
